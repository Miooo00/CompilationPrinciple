...
def analy_input_string(GS, table, state, input):
    ...
    # 用来打印输出顺序
    show_count = 1
    meg += f'{"%s %8s %8s %8s %8s" % ("步骤", "栈", "优先关系", "剩余输入串", "移进或规约")}' + '\n'
    print("%s %8s %8s %8s %8s" % ("步骤", "栈", "优先关系", "剩余输入串", "移进或规约"))
    while (True):
        meg += str(show_count)
        print(show_count, end="")
        show_count += 1
        meg += f'{"%12s" % ("".join(ana_shed))}'
        ...
        meg += f'{"%8s" % (relationship)}'
        print("%8s" % (relationship), end="")

        meg += f'{"%14s" % "".join(list((reversed("".join(input_shed)))))}'
...
def exe_entry(input_str="i+i#"):
    ...
    str_table += f'{"% 24s" % ("算符优先关系表")}'+'\n'
    str_table += '---------------------------------------------------------' + '\n'
    for i in state:
        str_table += f'{"%8s" % i}'
    str_table += '\n'
    for i in range(len(table)):
        str_table += f'{"%s" % state[i]}'
        str_table += f'{"%7s" % table[i][0]}'
        for x in table[i][1:]:
            str_table += f'{"%8s" % x}'
        str_table += '\n'
    ...
    # 对文法进行算符分析
    str_process += f'{"接下来对输入串 %16s 进行规约" % (input_str)}' + '\n'
    str_process += '---------------------------------------------------' + '\n'
    print("接下来对输入串 %16s 进行规约" % (input_str))
    print("---------------------------------------------------")
    meg = analy_input_string(GS, table, state, input_str)
    str_process += meg
    str_table += '\n' + str_process
    return str_table
